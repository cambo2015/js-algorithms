 ...
 */
 const insertionSort = (arr) => {
    ...
    for (let i = 1; i < arr.length; i++) { //0 wont run anyway in the while loop so skip it
        let j = i  //this is the starting point. From here we will go backwards
        while (j > 0 && arr[j - 1] > arr[j]) {  //nothing below zero. We compare the two arr[j-1] and arr[j] 
